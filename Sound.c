...
void PortB_Init(void){
	...
	//Initialize DAC
	GPIO_PORTB_DIR_R |= (PB0 | PB1 | PB2 | PB3);
	GPIO_PORTB_DEN_R |= (PB0 | PB1 | PB2 | PB3);
	GPIO_PORTB_AMSEL_R &= ~(PB0 | PB1 | PB2 | PB3);
	GPIO_PORTB_AFSEL_R &= ~(PB0 | PB1 | PB2 | PB3);
	GPIO_PORTB_PCTL_R &= ~(0x0000FFFF);
	
}

unsigned long Length;
const unsigned char *SoundPt;

//Outputs the bits to the R-2R network
//Input: The array containing the sine wave structure & what index
//Output: Writes binary value to GPIO_PORTB
...
void Sound_Init(void){
	Length = 0;
	Timer0_Init(&SoundTask,80000000/11000);
}

void Sound_Play(const unsigned char *pt, unsigned long count){
	Length = count;	//Records the size of sound array
	SoundPt = pt;
	NVIC_EN0_R = 1<<19;
}
